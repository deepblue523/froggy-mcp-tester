...
<#
.SYNOPSIS
  Bump the minor version in package.json (and package-lock.json) using npm semver rules.

.DESCRIPTION
  Runs `npm version minor --no-git-tag-version`, then commits the two files unless -NoCommit is set.
...
$v = Invoke-NpmSemverBump -Kind minor -RepoRoot $repoRoot -DryRun:$DryRun -NoCommit:$NoCommit
if (-not $DryRun) {
    Write-Host "Minor version is now $v."
}
